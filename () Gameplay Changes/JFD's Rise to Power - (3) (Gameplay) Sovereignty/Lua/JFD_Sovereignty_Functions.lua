...
function JFD_Sovereignty_PlayerDoTurn(playerID)
	...
	if (not hasGovernment) then 
		if numPopulation >= defineDefaultLimitedGovernmentPopThreshold then
			if player:IsHuman() then
				...
				player:InitiateGovernmentChoice(governmentChiefdomID, true)
			end
		end
	...
	if hasGovernment then
		...
		if (not Player.HasGovernment) then
			player:ChangeCurrentReformCooldown(defineDefaultReformCooldownRate)
		end	

		--GOVERNMENT COOLDOWN
		if (not Player.HasGovernment) then
			player:ChangeCurrentGovernmentCooldown(defineDefaultGovernmentCooldownRate)
		end	
	...
	--DICTATORSHIP
	if (not Player.HasGovernment) then
		if player:IsHasGovernment(governmentDictatorshipID) then
			...
			if dominantFactionID == -1 then
				...
				local prereqIdeology = GameInfo.JFD_Factions[dominantFactionID].PrereqIdeology
				local prereqIdeologyID = GameInfoTypes[prereqIdeology]
				if prereqIdeologyID then
					if prereqIdeologyID ~= player:GetIdeology() then
						player:ResetLegislature(governmentDictatorshipID)
					end
				end
			end
...
function JFD_Sovereignty_IdeologyAdopted(playerID, newIdeologyID)
	...
	if player:IsHasGovernment() then
		--DICTATORSHIP
		if player:GetCurrentGovernment() == governmentDictatorshipID then
			player:ResetLegislature(governmentDictatorshipID)
		end
...
local function JFD_Sovereignty_CityConstructed(playerID, cityID, buildingID)
	...
	if g_Buildings_UnlocksGovernment_Table[buildingID] then
		local currentGovernmentID = player:GetCurrentGovernment()
		if (not GameInfo.JFD_Governments[currentGovernmentID].IsUnique) then
			player:InitiateGovernmentChoice(nil, true)
		end
	end
...
local function JFD_Sovereignty_GetScenarioDiploModifier1(playerID, otherPlayerID)
	...
	elseif otherPlayer:HasReform(reformPrecedenceRankID) then
		if otherPlayer:IsPlayerHasOpenBorders(otherPlayerID) then
			numDiploMod = -10
		else
			numDiploMod = 10
		end
