...
--	if numUnlockedBranches > 0 then
		...
		for row in GameInfo.PolicyBranchTypes("PurchaseByLevel = 0 AND Type != 'POLICY_BRANCH_JFD_PHILOSOPHY'") do
	...
	while policyInfo ~= nil do
		...
		if (iBranch ~= nil and iBranch ~= "POLICY_BRANCH_JFD_IDEOLOGY" and iBranch ~= "POLICY_BRANCH_JFD_PHILOSOPHY" and not GameInfo.PolicyBranchTypes[iBranch].PurchaseByLevel) then
...
local g_DefaultPanelID = g_IdeologyPanelID
local g_CurrentPanelID = g_PhilosophyPanelID or g_DefaultPanelID
...
function UpdateSocialPolicyPanel()
	...
	local nextPanel = GameInfo.SocialPolicyPanels[g_CurrentPanelID+1]
	if nextPanel then
	...
	local prevPanel = GameInfo.SocialPolicyPanels[g_CurrentPanelID-1]
	if prevPanel then
...
function OnPrevScreenButton()
	g_CurrentPanelID = math.max(g_CurrentPanelID - 1,g_DefaultPanelID)
...
function OnNextScreenButton()
	g_CurrentPanelID = math.min(g_CurrentPanelID + 1,g_SocialPolicyPanels_Count)
