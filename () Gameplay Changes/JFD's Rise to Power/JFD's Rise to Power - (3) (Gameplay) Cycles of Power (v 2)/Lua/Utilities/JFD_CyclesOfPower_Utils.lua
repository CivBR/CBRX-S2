...
local gameSpeedMod		= gameSpeed.GrowthPercent
...
function Player.InitiateCycleOfPowerChoice(player, isReloading)
	...
	if playerIsHuman then
		...
		for index = 1, numCyclePowers do
			...
			if (not row.IsAnarchy) and flavour > cyclePowerPrefVal then
...
function Player.GetVirtueRate(player, cyclePowerID)	
	...
	if currentValRatePerCity > 0 then
		currentValRatePerCity = g_MathFloor(player:GetNumCities()/currentValRatePerCity) * currentValRate
	end

	local currentValRatePerXPopulation = cyclePower.VirtueRatePerXPopulation
	if currentValRatePerXPopulation > 0 then
		currentValRatePerXPopulation = g_MathFloor(player:GetTotalPopulation()/currentValRatePerXPopulation) * currentValRate
	end
	...
	if currentValRatePerXCapitalPopulation > 0 then
		currentValRatePerXCapitalPopulation = g_MathFloor(capital:GetPopulation()/currentValRatePerXCapitalPopulation) * currentValRate
	end
	...
	if player:HasTheocracyCycle() then
		...
		if currentValRatePerCityNotFollowing > 0 then
			...
			if religionID > 0 then
				...
				currentValRatePerCityNotFollowing = g_GetRound(numCitiesNotFollowing/currentValRatePerCityNotFollowing) * currentValRate
			end
		end
	end
...
function Player_GetVirtueModifier(player, cyclePowerID)	
	local currentValMod = (gameSpeedMod-100)
