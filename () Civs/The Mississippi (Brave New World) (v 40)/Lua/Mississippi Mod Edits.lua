--Tomatekh
print ("Mississippi Edits")

--Stuff
local bMaint = GameInfoTypes.BUILDINGCLASS_MISSISSIPPI_MAINT_DUMMY;
...
function UpdatedGoldTipHandler( control )
	...
	local iMaintRefund = pPlayer:GetBuildingClassCount(bMaint);

	local iBuildingMaintenance = iBuildingMaintenanceBase;

	if (iBuildingMaintenanceBase - iMaintRefund) > 0 then
		iBuildingMaintenance = (iBuildingMaintenanceBase - iMaintRefund);
	elseif (iBuildingMaintenanceBase - iMaintRefund) <= 0 then
...
function UpdateGoldTopPanelInfo()
	...
	if not (pPlayer:GetCivilizationType() == GameInfoTypes.CIVILIZATION_MISSISSIPPI_MOD_TOMATEKH) then			
		return
	end
	...
	if iGPTBase >= 0 then
		iGoldPerTurn = (pPlayer:CalculateGoldRate() + iMaintRefund)
	elseif iGPTBase < 0 then
		if (iGPTBase + iMaintRefund) < 0 then
			iGoldPerTurn = (pPlayer:CalculateGoldRate() + iMaintRefund)
		elseif (iGPTBase + iMaintRefund) >= 0 then 
