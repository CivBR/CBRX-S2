...
local LandInfantry = { 
	...
	GameInfo.Units.UNIT_MC_MARATHAN_SEPOY.ID, 
...
local iConscriptionID = GameInfo.Units.UNIT_MC_CONSCRIPTION.ID
                ...
                if (pUnit:GetUnitType() == iConscriptionID) then
					...
					if (uPlot:IsCity()) then
						...
						if NumCitizens > 2 then
							...
							pUnit:Kill();
							local title =  Locale.ConvertTextKey("TXT_KEY_NOMC_CONSCRIPTION_NOTIFICATION_TITLE");
							local descr =  Locale.ConvertTextKey("TXT_KEY_NOMC_CONSCRIPTION_NOTIFICATION");
